...
namespace App\Http\Controllers\Modules\support\Users\Staff;
...
class Staff extends Controller
{
    public function StaffIndex(Request $request)
    {
        ...
        $query = DB::table('users')
            ...
            ->whereIn('users.campus_id', $userCampusIds) // ✳️ Always limit to allowed campuses
            ->select(
                'users.id as user_id',
        ...
        if ($request->filled('campus_id')) {
            ...
            if (in_array($campusId, $userCampusIds)) {
                $query->where('users.campus_id', $campusId);
            } else {
                // Force empty result if trying to query unauthorized campus
                $query->whereRaw('0 = 1');
            }
        }
        ...
        return view('modules.support.users.staff.index', compact(
            'staffUsers',
            'titles',
            'departments',
            'positions',
    ...
    public function StaffView($uid)
    {
        ...
        return view('modules.support.users.staff.view', compact('staff', 'campuses', 'loginLogs', 'sessions', 'GeneralLogs'));
    }
    ...
    public function StaffEdit(Request $request, $uid)
    {
        ...
        return view('modules.support.users.staff.edit', compact(
            'staff',
            'disabilities',
            'allCampuses',
            'userCampuses',
    ...
    public function StaffUpdate(Request $request, $uid)
    {
        ...
        $validated = $request->validate([
            ...
            'status'      => 'required|in:inactive,suspended',
    ...
    public function StaffCreate()
    {
        ...
        $allWorkspaces = DB::table('workspaces')->where('id', '3')->pluck('display_name', 'id');

        return view('modules.support.users.staff.create', compact(
            'titles',
            'departments',
            'positions',
            'disabilities',
    ...
    public function StaffStore(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('support.users.staff')->with('success', 'Staff account created successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
            report($e);
    ...
    {
        // Step 1: Retrieve the staff_deletion system setting
        ...
        try {
            ...
            return redirect()->route('support.users.staff')->with('success', 'User deleted successfully');
    ...
    public function StaffPermissions($uid)
{
    ...
    return view('modules.support.users.staff.permissions', compact(
        'staff',
        'workspaces',
...
public function StaffUpdatePermissions(Request $request, $uid)
{
    ...
    if ($request->filled('permissions')) {
        $insert = [];
        foreach (array_unique($request->permissions) as $perm) {
            $insert[] = [
                'user_id' => $staff->id,
                'permission_id' => $perm,
